...
p_load('tidyverse', 'jsonlite', 'magrittr', 'xlsx',
       'stargazer', 'psych', 'jtools', 'DT', 'ggstatsplot', 'lavaan')
...
boxplot(questionnaire$mean_CMQ ~ questionnaire$socio_sex)

# Calculate mean and standard deviation
mu <- mean(questionnaire$mean_CMQ, na.rm = TRUE)
sigma <- sd(questionnaire$mean_CMQ, na.rm = TRUE)

# Create the histogram with normal distribution overlay
ggplot(questionnaire, aes(x = mean_CMQ)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, fill = "dodgerblue3", color = "white") +
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma), color = "red") +
  labs(x = "Mean Affection Imagery", y = "Density") +
  theme_apa() +
