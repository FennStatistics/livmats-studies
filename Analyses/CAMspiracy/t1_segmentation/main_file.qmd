...
## Republicans
suppressMessages(read_file('jatos_results_data_20231125181813_Republicans.txt') %>%
  # ... split it into lines ...
  str_split('\n') %>% first() %>%
  # ... filter empty rows ...
  discard(function(x) x == '') %>%
...
prolific_Republican <- read.csv(file = "prolific_export_65620efdd4432eb81c6a1206_Republican.csv", header = TRUE)

prolific <- rbind(prolific, prolific_Republican)
...
```{r}
tmp_dat <- questionnaire[questionnaire$feedback_critic != "" & !is.na(questionnaire$feedback_critic), c("politicalParty", "feedback_critic")]
