...
if (all(questionnaire$PROLIFIC_PID == networkIndicators_pre$participantCAM) &
  ...
  ## save as .xlsx file
  xlsx::write.xlsx2(x = networkIndicators_pre, file = "networkIndicators_pre_clean_final.xlsx")
  ## save as R object
  saveRDS(networkIndicators_pre, file = "networkIndicators_pre_clean_final.rds")
  
  # save network indicators post
  ## save as .xlsx file
  xlsx::write.xlsx2(x = networkIndicators_post, file = "networkIndicators_post_clean_final.xlsx")
  ## save as R object
  saveRDS(networkIndicators_post, file = "networkIndicators_post_clean_final.rds")
...
# i <- 1170
# tmp_nodes$text_summarized[stringdist::stringdist(a = dat_overwrite$Words[i], tmp_nodes$text_summarized) <= 2]
  ...
  if(!is.na(dat_overwrite$Superordinate[i])){
    ...
    for(j in 1:nrow(tmp)){
      if(sum(tmp_nodes$id %in% tmp[j,]$id) != 1){
        ...
        # add suffix
        if(tmp[j,]$value < 0){
          tmp[j,]$text_summarized <- paste0(dat_overwrite$Superordinate[i], "_negative")
        }else if(tmp[j,]$value == 0){
          tmp[j,]$text_summarized <- paste0(dat_overwrite$Superordinate[i], "_neutral")
        }else if(tmp[j,]$value == 10){
          tmp[j,]$text_summarized <- paste0(dat_overwrite$Superordinate[i], "_ambivalent")
        }else{
          tmp[j,]$text_summarized <- paste0(dat_overwrite$Superordinate[i], "_positive")
        }
      }   
    }        
...
table(tmp_nodes$text_summarized)


table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[1]])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[2]])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[3]])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[4]])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[5]])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == unique(tmp_nodes$participantCAM)[6]])

CAMfiles_pre[[1]][CAMfiles_pre[[1]]$participantCAM == unique(CAMfiles_pre[[1]]$participantCAM)[6],]
CAMfiles_post[[1]][CAMfiles_post[[1]]$participantCAM == unique(CAMfiles_post[[1]]$participantCAM)[6],]




table(questionnaire$typeChange)
questionnaire$PROLIFIC_PID[questionnaire$typeChange == "A"]

table(tmp_nodes$CAM[tmp_nodes$participantCAM == "6480b160d51d051b42f67b3d"])
table(tmp_nodes$CAM[tmp_nodes$participantCAM == "6480b160d51d051b42f67b3d"])

```


## split data sets robots
